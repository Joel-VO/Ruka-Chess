...
use evaluation::evaluations::additional_eval_capability;
// use evaluation::evaluations::{evaluation_func,pe_sto};
fn main() {
    println!("Enter fen string ");
    let max_time = Duration::new(2,0);//seconds and nano-seconds adjustments
    ...
    let piece_to_move = board_fen[1];//takes just the current player
        ...
        Ok(board) => {//checks condition to see if board is legal
            ...
            let eval = additional_eval_capability(&board, 32,24);
            println!("{eval}");
        }
        Err(err) => {
