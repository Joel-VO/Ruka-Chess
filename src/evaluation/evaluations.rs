...
pub fn evaluate(board:&Board)->i32{
    ...
    for ch in parse_fen.chars(){//the piece tables are skewing the results, so have to change them
        ...
        match ch{
            ...
            'p' => {
                let val = black_pawn_piece_table[7-row][7-col];
                evaluation -= 100 + val;
                col+=1;
            },
            'N' => {
                let val = white_knight_piece_table[row][col];
                evaluation += 320 + val;
                col+=1;
            },
            'n' => {
                let val = black_knight_piece_table[7-row][7-col];
                evaluation -= 320 + val;
                col+=1;
            },
            'B' => {
                let val = white_bishop_piece_table[row][col];
                evaluation += 330 + val;
                col+=1;
            },
            'b' => {
                let val = black_bishop_piece_table[7-row][7-col];
                evaluation -= 330 + val;
                col+=1;
            },
            'R' => {
                let val = white_rook_piece_table[row][col];
                evaluation += 500 + val;
                col+=1;
            },
            'r' => {
                let val = black_rook_piece_table[7-row][7-col];
                evaluation -= 500 + val;
                col+=1;
            },
            'Q' => {
                let val = white_queen_piece_table[row][col];
                println!("{val}, {ch}, {row} {col}");
                evaluation += 900 + val;
                col+=1;
            },
            'q' => {
                let val = black_queen_piece_table[7-row][7-col];//so... queens and kings being non symmetric
